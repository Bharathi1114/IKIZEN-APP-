...
def generate_food_data(base_foods, preparations, target_count):
    foods = []
    count = 0
    ...
    while count < target_count:
        for prep in preparations:
            for base in base_foods:
                ...
                name = f"{prep} {base['name']}".strip()
                cal = round(base['cal'] * (1 + random.uniform(-0.1, 0.2)))
                prot = round(base['prot'] * (1 + random.uniform(-0.1, 0.1)), 1)
                carb = round(base['carb'] * (1 + random.uniform(-0.1, 0.1)), 1)
                fat = round(base['fat'] * (1 + random.uniform(-0.1, 0.2)), 1)
...
preparations = ["", "Boiled", "Grilled", "Roasted", "Fried", "Steamed", "Baked", "Spicy", "Crispy", "Pan-seared", "Smoked", "Slow-cooked"]
...
veg_base = [
    ...
    {"name": "Paneer", "cal": 298, "prot": 18, "carb": 3.4, "fat": 24},
...
exercise_variants = ["", "Incline", "Decline", "Weighted", "Banded", "Single-arm", "Single-leg", "Isometric", "Explosive", "Slow Negative", "Pause"]

def generate_exercises(target_count):
    exercises = []
    count = 0
    
    while count < target_count:
        for variant in exercise_variants:
            for base in exercise_base:
                if count >= target_count:
                    break
                
                name = f"{variant} {base['name']}".strip()
                exercises.append({
                    'id': str(uuid.uuid4()),
                    'name': name.replace("'", "''"),
                    'category': base['category'],
                    'muscle_group': base['muscle'],
                    'description': f"A {base['diff'].lower()} level {base['category']} exercise targeting the {base['muscle'].lower()}.",
...
veg_foods = generate_food_data(veg_base, preparations, num_records)
non_veg_foods = generate_food_data(non_veg_base, preparations, num_records)
vegan_foods = generate_food_data(vegan_base, preparations, num_records)
keto_foods = generate_food_data(keto_base, preparations, num_records)
exercises_data = generate_exercises(num_records)
...
def generate_inserts(table, data):
    ...
    for item in data:
        if table == 'exercises':
            inserts += f"INSERT INTO {table} (id, name, category, muscle_group, description, difficulty) VALUES ('{item['id']}', '{item['name']}', '{item['category']}', '{item['muscle_group']}', '{item['description']}', '{item['difficulty']}');\n"
        else:
            inserts += f"INSERT INTO {table} (id, name, calories, protein, carbs, fat, serving_size) VALUES ('{item['id']}', '{item['name']}', {item['calories']}, {item['protein']}, {item['carbs']}, {item['fat']}, '{item['serving_size']}');\n"
    return inserts

sql += generate_inserts('veg_foods', veg_foods)
sql += generate_inserts('non_veg_foods', non_veg_foods)
sql += generate_inserts('vegan_foods', vegan_foods)
sql += generate_inserts('keto_foods', keto_foods)
sql += generate_inserts('exercises', exercises_data)

with open('seed.sql', 'w', encoding='utf-8') as f:
    f.write(sql)

print('Successfully created seed.sql with 1500+ records.')
